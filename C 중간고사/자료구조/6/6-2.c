...
int isFull(Queue *q){ // 큐가 가득 차 있다면 rear은 어떤 상태?
...
int isEmpty(Queue *q){ // 큐가 비어 있따면 rear과 front 는 어떤 상태일까?
...
void printAll(Queue *q){ // 0 부터 배열크기-1 에서  front+1 ~rear사이의 데이터만 출력
